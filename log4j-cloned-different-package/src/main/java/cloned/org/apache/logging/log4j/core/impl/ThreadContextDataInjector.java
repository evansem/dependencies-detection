 ...
 * {@code org.apache.logging.log4j.ThreadContext} map implementations into a {@code StringMap}. In the case of duplicate keys,
...
public class ThreadContextDataInjector {
    ...
    /**
     * Default {@code ContextDataInjector} for the legacy {@code Map<String, String>}-based org.apache.logging.log4j.ThreadContext (which is
     * also the org.apache.logging.log4j.ThreadContext implementation used for web applications).
     * <p>
    ...
    public static class ForDefaultThreadContextMap implements ContextDataInjector {
        ...
        public StringMap injectContextData(final List<Property> props, final StringMap contextData) {
            ...
            // This is a copy-on-write data structure so we are sure org.apache.logging.log4j.ThreadContext changes will not affect our copy.
            ...
            // If the list of Properties is non-empty we need to combine the properties and the org.apache.logging.log4j.ThreadContext
            ...
            // the org.apache.logging.log4j.ThreadContext - this will throw an UnsupportedOperationException if we try to modify it.
    ...
    public static class ForGarbageFreeThreadContextMap implements ContextDataInjector {
        ...
        public StringMap injectContextData(final List<Property> props, final StringMap reusable) {
            // When the org.apache.logging.log4j.ThreadContext is garbage-free, we must copy its key-value pairs into the specified reusable
            // StringMap. We cannot return the org.apache.logging.log4j.ThreadContext's org.apache.logging.log4j.internal data structure because it may be modified later
     ...
     * StringMap-based data structure.
     * <p>
     * If there are no configuration properties, this injector will return the thread context's org.apache.logging.log4j.internal data
    ...
    public static class ForCopyOnWriteThreadContextMap implements ContextDataInjector {
        ...
        public StringMap injectContextData(final List<Property> props, final StringMap ignore) {
            // If there are no configuration properties we want to just return the org.apache.logging.log4j.ThreadContext's StringMap:
            // it is a copy-on-write data structure so we are sure org.apache.logging.log4j.ThreadContext changes will not affect our copy.
            ...
            // However, if the list of Properties is non-empty we need to combine the properties and the org.apache.logging.log4j.ThreadContext
            // data. Note that we cannot reuse the specified StringMap: some Loggers may have properties defined
            // and others not, so the LogEvent's context data may have been replaced with an immutable copy from
            // the org.apache.logging.log4j.ThreadContext - this will throw an UnsupportedOperationException if we try to modify it.
