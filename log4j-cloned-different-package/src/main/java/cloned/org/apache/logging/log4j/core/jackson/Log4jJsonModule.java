...
class Log4jJsonModule extends SimpleModule {
    ...
    public void setupModule(final SetupContext context) {
        ...
        if (encodeThreadContextAsList) {
            new Initializers.SetupContextInitializer().setupModule(context, includeStacktrace, stacktraceAsString);
        } else {
            new Initializers.SetupContextJsonInitializer().setupModule(context, includeStacktrace, stacktraceAsString);
        }
    }
}
