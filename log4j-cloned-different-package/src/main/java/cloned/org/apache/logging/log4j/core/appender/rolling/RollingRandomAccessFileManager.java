 ...
 */
package cloned.org.apache.logging.log4j.core.appender.rolling;
...
import cloned.org.apache.logging.log4j.core.appender.ConfigurationFactoryData;
...
import cloned.org.apache.logging.log4j.core.util.NullOutputStream;
...
public class RollingRandomAccessFileManager extends RollingFileManager {
    ...
    public static RollingRandomAccessFileManager getRollingRandomAccessFileManager(final String fileName,
        ...
        return AbstractManager.narrow(RollingRandomAccessFileManager.class, OutputStreamManager.getManager(name, new FactoryData(fileName, filePattern, isAppend,
            ...
            if (data.fileName != null) {
                ...
                try {
                    ...
                    if (data.append) {
                        ...
                        AbstractManager.LOGGER.trace("RandomAccessFile {} seek to {}", name, length);
                        raf.seek(length);
                    } else {
                        AbstractManager.LOGGER.trace("RandomAccessFile {} set length to 0", name);
                        raf.setLength(0);
                    }
                } catch (final IOException ex) {
                    AbstractManager.LOGGER.error("Cannot access RandomAccessFile " + ex, ex);
                    if (raf != null) {
                        try {
                            raf.close();
                        } catch (final IOException e) {
                            AbstractManager.LOGGER.error("Cannot close RandomAccessFile {}", name, e);
                        }
                    }
                    return null;
                }
