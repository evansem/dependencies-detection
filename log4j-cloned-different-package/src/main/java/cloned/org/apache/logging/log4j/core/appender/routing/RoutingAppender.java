...
import cloned.org.apache.logging.log4j.core.appender.AbstractAppender;
import cloned.org.apache.logging.log4j.core.appender.rewrite.RewritePolicy;
...
public final class RoutingAppender extends AbstractAppender {
    ...
    public static class Builder<B extends Builder<B>> extends AbstractAppender.Builder<B>
            implements cloned.org.apache.logging.log4j.core.util.Builder<RoutingAppender> {
        ...
        public RoutingAppender build() {
            ...
            if (name == null) {
                AbstractLifeCycle.LOGGER.error("No name defined for this RoutingAppender");
                return null;
            }
            if (routes == null) {
                AbstractLifeCycle.LOGGER.error("No routes defined for RoutingAppender {}", name);
                return null;
            }
    ...
    public void deleteAppender(final String key) {
        AbstractLifeCycle.LOGGER.debug("Deleting route with {} key ", key);
        ...
        if (null != control) {
            AbstractLifeCycle.LOGGER.debug("Stopping route with {} key", key);
            ...
            if (referencedAppenders.containsKey(key)) {
                AbstractLifeCycle.LOGGER.debug("Route {} using an appender reference may not be removed because " +
                        "the appender may be used outside of the RoutingAppender", key);
            } else {
                AbstractLifeCycle.LOGGER.debug("Route with {} key already deleted", key);
            }
        }
    }
    ...
    public static RoutingAppender createAppender(
        ...
        if (name == null) {
            AbstractLifeCycle.LOGGER.error("No name provided for RoutingAppender");
            return null;
        }
        if (routes == null) {
            AbstractLifeCycle.LOGGER.error("No routes defined for RoutingAppender");
            return null;
        }
    ...
    private static final class CreatedRouteAppenderControl extends RouteAppenderControl {
        ...
        void checkout() {
            if (pendingDeletion) {
                AbstractLifeCycle.LOGGER.warn("CreatedRouteAppenderControl.checkout invoked on a " +
        ...
        void tryStopAppender() {
            if (pendingDeletion
                ...
                AbstractLifeCycle.LOGGER.debug("Stopping appender {}", appender);
                appender.stop();
            }
        }
    }
